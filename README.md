  ...
  <div class="phone">
    ...
    <div class="content">
      <section id="panel-doc">
        <div class="card">
          <label class="drop">
            <input class="file" id="frontInput" type="file" accept="image/jpeg,image/png">
        ...
        <div class="card">
          <label class="drop">
            <input class="file" id="backInput" type="file" accept="image/jpeg,image/png">
...
<script>
  ...
  function bind(inputId,imgId){
    ...
    input.addEventListener('change',()=>{
      ...
      let url=URL.createObjectURL(f),fallback=false
      const show=()=>{img.style.display='block';const ph=box.querySelector('.ph');if(ph)ph.style.display='none';const hint=box.querySelector('.hint');if(hint)hint.style.display='none'}
      img.onload=()=>{if(url)URL.revokeObjectURL(url);show()}
      img.onerror=()=>{if(!fallback){fallback=true;if(url){URL.revokeObjectURL(url);url=null}const r=new FileReader();r.onload=e=>{img.src=e.target.result};r.onerror=()=>{input.value='';toast('Не удалось прочитать файл',false)};r.readAsDataURL(f)}else{input.value='';toast('Не удалось отобразить изображение. Используйте JPG/PNG.',false)}}
      img.src=url
    })
  }
  ...
  $('btn-send-req').onclick=()=>{const data={fio:$('fio')?.value?.trim(),iin:$('iin')?.value?.trim(),dob:$('dob')?.value,docnum:$('docnum')?.value?.trim(),issued:$('issued')?.value,expiry:$('expiry')?.value};console.log('Отправка реквизитов',data);toast('Реквизиты подготовлены к отправке (демо).',true)}
  showDoc()
</script>
</body>
</html>
